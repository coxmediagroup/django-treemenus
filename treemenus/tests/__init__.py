...
class TreemenuTemplateTestCase(TemplateTestCase):
    ''' Site-aware Treemenu Test '''

    def setUp(self):
        ''' set up regex and site '''
        self.site = Site.objects.create(name="originator.com", domain="originator.com")
        self.site.save()
        self.html_re = re.compile(r'<[^>]*>')
        self.spaces_re = re.compile(r'\s+')

    def test_templatetag_by_originating_site(self):
        ''' test template tag with origin '''

        menu = Menu.objects.create(
            name="origin_menu",
            originating_site=self.site,
            )
        caption1 = 'menu_caption1'
        caption2 = 'menu_caption2'
        menu.save()

        menu_item1 = MenuItem.objects.create(caption=caption1,
                                             parent=menu.root_item)
        menu_item1.save()
        menu_item2 = MenuItem.objects.create(caption=caption2,
                                             parent=menu.root_item)
        menu_item2.save()

        ex = self.render(
            '{% load tree_menu_tags %}' +
            '{% show_menu "origin_menu" "originator_com" "unordered-list" %}'
            )

        # strip html and spaces because the core treemenu template may
        # change
        ex = self.html_re.sub('', ex)
        ex = self.spaces_re.sub(' ', ex)

        self.assertEqual(ex, ' %s %s ' % (caption1, caption2))
